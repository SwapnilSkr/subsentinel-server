import { type IDeviceToken, DeviceToken, User } from "../models";
 ...
 */
export async function registerDevice(data: DeviceData): Promise<IDeviceToken> {
 ...
 */
export async function getUserDevices(userId: string): Promise<IDeviceToken[]> {
	if (!Types.ObjectId.isValid(userId)) {
		return [];
	}
